...
Se $F$ é um campo escalar, $\nabla F$ é o \textit{gradiente} de $F$. Considere um deslocamento na direção do vetor unitário $\mathbf{u}$. A variação $dF$ é dada por 
\begin{equation*}
F(\mathbf{P}+ \mathbf{u}dt) - F(\mathbf{P}) \equiv dF = \partial_i F \, dx^i = dt(u \cdot \nabla F),
\end{equation*}
\noindent portanto, $\nabla F$ aponta na direção de maior taxa de variação de $F$, e é normal à superfícies $F =$ constante.

Se $\mathbf{F} = F^i \mathbf{e}_i$ é um campo vetorial, $\nabla \cdot \mathbf{F}$ é a \textit{divergência} de $\mathbf{F}
$, e $\nabla \times \mathbf{F}$ é seu \textit{rotacional}. A divergência em um ponto $\mathbf{P}$ é definida como 
\begin{equation}
\nabla \cdot \mathbf{F} \equiv \lim\limits_{V \to 0} \frac{\Phi_F(S)}{V} ,
\label{def: div}
\end{equation}

\noindent onde $V$ é o volume da região delimitada por $S$, e $\mathbf{P}$ é um ponto dentro dessa região. Se $\nabla \cdot \mathbf{F} > 0$, então $\mathbf{P}$ é uma fonte de $\mathbf{F}$; se $\nabla \cdot \mathbf{F} < 0$, então $\mathbf{P}$ é um sorvedouro de $\mathbf{F}$. Em coordenadas cartesianas, a expressão para a divergência de um campo vetorial é $\nabla \cdot \mathbf{F} = \partial_i F^i$.
