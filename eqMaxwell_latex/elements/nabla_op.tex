...
\end{equation*}
\noindent portanto, $\nabla F$ aponta na direção de maior taxa de variação de $F$, e é normal à superfícies $F =$ constante. Na equação acima e no resto desse apêndice, foi utilizada a notação de Eistein, onde índices repetidos em posições diferentes (em cima e em baixo) significam que há um somatório implícito sobre esse índice.
...
Também definimos o \textit{laplaciano} de um campo escalar como sendo
\begin{equation}
    \nabla^2 F = \nabla \cdot \nabla F \;.
\end{equation}

\noindent Enquanto que para um campo vetorial, $\nabla^2 \mathbf F = \nabla^2 (F^i \mathbf{e}_i) = (\nabla^2 F^i) \mathbf{e}_i$.

As seguinte identidades serão úteis:
\begin{equation}
